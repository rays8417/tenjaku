...
router.post('/tournaments', async (req, res) => {
    try {
        const { name, description, matchDate, team1, team2, venue, entryFee, maxParticipants } = req.body;
...
router.put('/tournaments/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, matchDate, team1, team2, venue, entryFee, maxParticipants, status } = req.body;
...
router.get('/stats', async (req, res) => {
    try {
        const [totalUsers, totalTournaments, totalTeams, totalRewards, activeTournaments, completedTournaments, totalEarnings, totalSpent] = await Promise.all([
            prisma.user.count(),
