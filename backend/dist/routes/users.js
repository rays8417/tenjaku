    ...
    try {
        ...
        const stats = {
            ...
            averageScore: user.teams.length > 0
                ? user.teams.reduce((sum, team) => sum + team.scores.reduce((scoreSum, score) => scoreSum + Number(score.totalScore), 0), 0) / user.teams.length
                : 0,
            bestRank: user.teams.length > 0
                ? Math.min(...user.teams.map((team) => team.rewards[0]?.rank || 999))
                : null,
            totalRewards: user.teams.reduce((sum, team) => sum + team.rewards.reduce((rewardSum, reward) => rewardSum + Number(reward.amount), 0), 0)
        };
        res.json({
            success: true,
            stats
