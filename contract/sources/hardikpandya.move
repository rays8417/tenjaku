module higgs::HardikPandya {
    ...
    struct CoinCapabilities has key {
        mint_cap: MintCapability<HardikPandya>,
        burn_cap: BurnCapability<HardikPandya>,
        freeze_cap: FreezeCapability<HardikPandya>,
    }
    ...
    fun init_module(admin: &signer) {
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<HardikPandya>(
            admin,
            string::utf8(b"HardikPandya"),
            string::utf8(b"HPA"),
            8,
            true, // monitor_supply
        );
        ...
        coin::register<HardikPandya>(admin);
        ...
        let coins = coin::mint<HardikPandya>(initial_supply, &mint_cap);
    ...
    public entry fun mint(admin: &signer, to: address, amount: u64) acquires CoinCapabilities, TokenHolders {
        ...
        let current_supply_option = coin::supply<HardikPandya>();
        ...
        let coins = coin::mint<HardikPandya>(amount, &caps.mint_cap);
    ...
    public entry fun register(account: &signer) acquires TokenHolders {
        ...
        coin::register<HardikPandya>(account);
    ...
    public entry fun transfer(from: &signer, to: address, amount: u64) acquires TokenHolders {
        coin::transfer<HardikPandya>(from, to, amount);
    ...
    public fun balance(addr: address): u64 {
        coin::balance<HardikPandya>(addr)
    }
    ...
    public fun get_coin_info(): (string::String, string::String, u8) {
        (
            coin::name<HardikPandya>(),
            coin::symbol<HardikPandya>(),
            coin::decimals<HardikPandya>()
        )
    }
