module higgs::ShubhamDube {
    ...
    struct CoinCapabilities has key {
        mint_cap: MintCapability<ShubhamDube>,
        burn_cap: BurnCapability<ShubhamDube>,
        freeze_cap: FreezeCapability<ShubhamDube>,
    }
    ...
    fun init_module(admin: &signer) {
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<ShubhamDube>(
            admin,
            string::utf8(b"ShubhamDube"),
            string::utf8(b"SDB"),
            8,
            true, // monitor_supply
        );
        ...
        coin::register<ShubhamDube>(admin);
        ...
        let coins = coin::mint<ShubhamDube>(initial_supply, &mint_cap);
    ...
    public entry fun mint(admin: &signer, to: address, amount: u64) acquires CoinCapabilities, TokenHolders {
        ...
        let current_supply_option = coin::supply<ShubhamDube>();
        ...
        let coins = coin::mint<ShubhamDube>(amount, &caps.mint_cap);
    ...
    public entry fun register(account: &signer) acquires TokenHolders {
        ...
        coin::register<ShubhamDube>(account);
    ...
    public entry fun transfer(from: &signer, to: address, amount: u64) acquires TokenHolders {
        coin::transfer<ShubhamDube>(from, to, amount);
    ...
    public fun get_coin_info(): (string::String, string::String, u8) {
        (
            coin::name<ShubhamDube>(),
            coin::symbol<ShubhamDube>(),
            coin::decimals<ShubhamDube>()
        )
    }
