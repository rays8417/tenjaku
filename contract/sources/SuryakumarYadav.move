module higgs::SuryakumarYadav {
    ...
    struct SuryakumarYadav {}

    struct CoinCapabilities has key {
        mint_cap: MintCapability<SuryakumarYadav>,
        burn_cap: BurnCapability<SuryakumarYadav>,
        freeze_cap: FreezeCapability<SuryakumarYadav>,
    }
    ...
    fun init_module(admin: &signer) {
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<SuryakumarYadav>(
            admin,
            string::utf8(b"SuryakumarYadav"),
            string::utf8(b"SKY"),
            8,
            true, // monitor_supply
        );
        ...
        coin::register<SuryakumarYadav>(admin);
        ...
        let coins = coin::mint<SuryakumarYadav>(initial_supply, &mint_cap);
    ...
    public entry fun mint(admin: &signer, to: address, amount: u64) acquires CoinCapabilities, TokenHolders {
        ...
        let current_supply_option = coin::supply<SuryakumarYadav>();
        ...
        let coins = coin::mint<SuryakumarYadav>(amount, &caps.mint_cap);
    ...
    public entry fun register(account: &signer) acquires TokenHolders {
        ...
        coin::register<SuryakumarYadav>(account);
    ...
    public entry fun transfer(from: &signer, to: address, amount: u64) acquires TokenHolders {
        coin::transfer<SuryakumarYadav>(from, to, amount);
    ...
    public fun balance(addr: address): u64 {
        coin::balance<SuryakumarYadav>(addr)
    }
    ...
    public fun get_coin_info(): (string::String, string::String, u8) {
        (
            coin::name<SuryakumarYadav>(),
            coin::symbol<SuryakumarYadav>(),
            coin::decimals<SuryakumarYadav>()
        )
    }
