module higgs::AbhishekSharma {
    ...
    struct AbhishekSharma {}

    struct CoinCapabilities has key {
        mint_cap: MintCapability<AbhishekSharma>,
        burn_cap: BurnCapability<AbhishekSharma>,
        freeze_cap: FreezeCapability<AbhishekSharma>,
    }
    ...
    fun init_module(admin: &signer) {
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<AbhishekSharma>(
            admin,
            string::utf8(b"AbhishekSharma"),
            string::utf8(b"ASH"),
            8,
            true, // monitor_supply
        );
        ...
        coin::register<AbhishekSharma>(admin);
        ...
        let coins = coin::mint<AbhishekSharma>(initial_supply, &mint_cap);
    ...
    public entry fun mint(admin: &signer, to: address, amount: u64) acquires CoinCapabilities, TokenHolders {
        ...
        let current_supply_option = coin::supply<AbhishekSharma>();
        ...
        let coins = coin::mint<AbhishekSharma>(amount, &caps.mint_cap);
    ...
    public entry fun register(account: &signer) acquires TokenHolders {
        ...
        coin::register<AbhishekSharma>(account);
    ...
    public entry fun transfer(from: &signer, to: address, amount: u64) acquires TokenHolders {
        coin::transfer<AbhishekSharma>(from, to, amount);
    ...
    public fun balance(addr: address): u64 {
        coin::balance<AbhishekSharma>(addr)
    }
    ...
    public fun get_coin_info(): (string::String, string::String, u8) {
        (
            coin::name<AbhishekSharma>(),
            coin::symbol<AbhishekSharma>(),
            coin::decimals<AbhishekSharma>()
        )
    }
