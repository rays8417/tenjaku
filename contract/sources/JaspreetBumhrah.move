module higgs::JaspreetBumhrah {
    ...
    struct JaspreetBumhrah {}

    struct CoinCapabilities has key {
        mint_cap: MintCapability<JaspreetBumhrah>,
        burn_cap: BurnCapability<JaspreetBumhrah>,
        freeze_cap: FreezeCapability<JaspreetBumhrah>,
    }
    ...
    fun init_module(admin: &signer) {
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<JaspreetBumhrah>(
            admin,
            string::utf8(b"JaspreetBumhrah"),
            string::utf8(b"JBH"),
            8,
            true, // monitor_supply
        );
        ...
        coin::register<JaspreetBumhrah>(admin);
        ...
        let coins = coin::mint<JaspreetBumhrah>(initial_supply, &mint_cap);
    ...
    public entry fun mint(admin: &signer, to: address, amount: u64) acquires CoinCapabilities, TokenHolders {
        ...
        let current_supply_option = coin::supply<JaspreetBumhrah>();
        ...
        let coins = coin::mint<JaspreetBumhrah>(amount, &caps.mint_cap);
    ...
    public entry fun register(account: &signer) acquires TokenHolders {
        ...
        coin::register<JaspreetBumhrah>(account);
    ...
    public entry fun transfer(from: &signer, to: address, amount: u64) acquires TokenHolders {
        coin::transfer<JaspreetBumhrah>(from, to, amount);
    ...
    public fun balance(addr: address): u64 {
        coin::balance<JaspreetBumhrah>(addr)
    }
    ...
    public fun get_coin_info(): (string::String, string::String, u8) {
        (
            coin::name<JaspreetBumhrah>(),
            coin::symbol<JaspreetBumhrah>(),
            coin::decimals<JaspreetBumhrah>()
        )
    }
