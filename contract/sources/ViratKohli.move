module higgs::ViratKohli {
    ...
    struct CoinCapabilities has key {
        mint_cap: MintCapability<ViratKohli>,
        burn_cap: BurnCapability<ViratKohli>,
        freeze_cap: FreezeCapability<ViratKohli>,
    }
    ...
    fun init_module(admin: &signer) {
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<ViratKohli>(
            admin,
            string::utf8(b"ViratKohli"),
            string::utf8(b"VKL"),
            8,
            true, // monitor_supply
        );
        ...
        coin::register<ViratKohli>(admin);
        ...
        let coins = coin::mint<ViratKohli>(initial_supply, &mint_cap);
    ...
    public entry fun mint(admin: &signer, to: address, amount: u64) acquires CoinCapabilities, TokenHolders {
        ...
        let current_supply_option = coin::supply<ViratKohli>();
        ...
        let coins = coin::mint<ViratKohli>(amount, &caps.mint_cap);
    ...
    public entry fun register(account: &signer) acquires TokenHolders {
        ...
        coin::register<ViratKohli>(account);
    ...
    public entry fun transfer(from: &signer, to: address, amount: u64) acquires TokenHolders {
        coin::transfer<ViratKohli>(from, to, amount);
    ...
    public fun get_coin_info(): (string::String, string::String, u8) {
        (
            coin::name<ViratKohli>(),
            coin::symbol<ViratKohli>(),
            coin::decimals<ViratKohli>()
        )
    }
