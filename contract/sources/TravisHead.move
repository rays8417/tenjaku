module higgs::TravisHead {
    ...
    struct CoinCapabilities has key {
        mint_cap: MintCapability<TravisHead>,
        burn_cap: BurnCapability<TravisHead>,
        freeze_cap: FreezeCapability<TravisHead>,
    }
    ...
    fun init_module(admin: &signer) {
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<TravisHead>(
            admin,
            string::utf8(b"TravisHead"),
            string::utf8(b"TSH"),
            8,
            true, // monitor_supply
        );
        ...
        coin::register<TravisHead>(admin);
        ...
        let coins = coin::mint<TravisHead>(initial_supply, &mint_cap);
    ...
    public entry fun mint(admin: &signer, to: address, amount: u64) acquires CoinCapabilities, TokenHolders {
        ...
        let current_supply_option = coin::supply<TravisHead>();
        ...
        let coins = coin::mint<TravisHead>(amount, &caps.mint_cap);
    ...
    public entry fun register(account: &signer) acquires TokenHolders {
        ...
        coin::register<TravisHead>(account);
    ...
    public entry fun transfer(from: &signer, to: address, amount: u64) acquires TokenHolders {
        coin::transfer<TravisHead>(from, to, amount);
    ...
    public fun get_coin_info(): (string::String, string::String, u8) {
        (
            coin::name<TravisHead>(),
            coin::symbol<TravisHead>(),
            coin::decimals<TravisHead>()
        )
    }
