module higgs::GlenMaxwell {
    ...
    struct CoinCapabilities has key {
        mint_cap: MintCapability<GlenMaxwell>,
        burn_cap: BurnCapability<GlenMaxwell>,
        freeze_cap: FreezeCapability<GlenMaxwell>,
    }
    ...
    fun init_module(admin: &signer) {
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<GlenMaxwell>(
            admin,
            string::utf8(b"GlenMaxwell"),
            string::utf8(b"GMX"),
            8,
            true, // monitor_supply
        );
        ...
        coin::register<GlenMaxwell>(admin);
        ...
        let coins = coin::mint<GlenMaxwell>(initial_supply, &mint_cap);
    ...
    public entry fun mint(admin: &signer, to: address, amount: u64) acquires CoinCapabilities, TokenHolders {
        ...
        let current_supply_option = coin::supply<GlenMaxwell>();
        ...
        let coins = coin::mint<GlenMaxwell>(amount, &caps.mint_cap);
    ...
    public entry fun register(account: &signer) acquires TokenHolders {
        ...
        coin::register<GlenMaxwell>(account);
    ...
    public entry fun transfer(from: &signer, to: address, amount: u64) acquires TokenHolders {
        coin::transfer<GlenMaxwell>(from, to, amount);
    ...
    public fun get_coin_info(): (string::String, string::String, u8) {
        (
            coin::name<GlenMaxwell>(),
            coin::symbol<GlenMaxwell>(),
            coin::decimals<GlenMaxwell>()
        )
    }
