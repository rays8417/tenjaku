...
module higgs::swap {
    ...
    */
    ...
    fun init_module(sender: &signer) {
        move_to(sender, SwapInfo {
            fee_to: ZERO_ACCOUNT,
            admin: DEFAULT_ADMIN,
            pair_created: account::new_event_handle<PairCreatedEvent>(sender),
        });
    }
    ...
    public(friend) fun create_pair<X, Y>(
        ...
        // Use the sender directly instead of resource account

        let lp_name: string::String = string::utf8(b"higgs-");
        ...
        if (string::length(&lp_name) > MAX_COIN_NAME_LENGTH) {
            lp_name = string::utf8(b"higgs LPs");
        };
        ...
        move_to<PairEventHolder<X, Y>>(
            sender,
            PairEventHolder {
                add_liquidity: account::new_event_handle<AddLiquidityEvent<X, Y>>(sender),
                remove_liquidity: account::new_event_handle<RemoveLiquidityEvent<X, Y>>(sender),
                swap: account::new_event_handle<SwapEvent<X, Y>>(sender)
            }
        );
    ...
    fun swap<X, Y>(
        ...
        let balance_x_adjusted = (balance_x as u128) * prec - (amount_x_in as u128) * 400u128;
        let balance_y_adjusted = (balance_y as u128) * prec - (amount_y_in as u128) * 400u128;
    ...
    public entry fun upgrade_swap(sender: &signer, metadata_serialized: vector<u8>, code: vector<vector<u8>>) acquires SwapInfo {
        ...
        let swap_info = borrow_global<SwapInfo>(RESOURCE_ACCOUNT);
        assert!(sender_addr == swap_info.admin, ERROR_NOT_ADMIN);
        code::publish_package_txn(sender, metadata_serialized, code);
    }
