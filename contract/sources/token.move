module higgs::Boson {
    ...
    struct CoinCapabilities has key {
        mint_cap: MintCapability<Boson>,
        burn_cap: BurnCapability<Boson>,
        freeze_cap: FreezeCapability<Boson>,
    }
    ...
    fun init_module(admin: &signer) {
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<Boson>(
            admin,
            string::utf8(b"Boson"),
            string::utf8(b"BSN"),
            8,
        ...
        let coins = coin::mint<Boson>(initial_supply, &mint_cap);
    ...
    public entry fun mint(admin: &signer, to: address, amount: u64) acquires CoinCapabilities, TokenHolders {
        ...
        let current_supply_option = coin::supply<Boson>();
        ...
        let coins = coin::mint<Boson>(amount, &caps.mint_cap);
    ...
    public entry fun transfer(from: &signer, to: address, amount: u64) acquires TokenHolders {
        coin::transfer<Boson>(from, to, amount);
