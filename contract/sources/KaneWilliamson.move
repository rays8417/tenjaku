module higgs::KaneWilliamson {
    ...
    struct KaneWilliamson {}

    struct CoinCapabilities has key {
        mint_cap: MintCapability<KaneWilliamson>,
        burn_cap: BurnCapability<KaneWilliamson>,
        freeze_cap: FreezeCapability<KaneWilliamson>,
    }
    ...
    fun init_module(admin: &signer) {
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<KaneWilliamson>(
            admin,
            string::utf8(b"KaneWilliamson"),
            string::utf8(b"KWS"),
            8,
            true, // monitor_supply
        );
        ...
        coin::register<KaneWilliamson>(admin);
        ...
        let coins = coin::mint<KaneWilliamson>(initial_supply, &mint_cap);
    ...
    public entry fun mint(admin: &signer, to: address, amount: u64) acquires CoinCapabilities, TokenHolders {
        ...
        let current_supply_option = coin::supply<KaneWilliamson>();
        ...
        let coins = coin::mint<KaneWilliamson>(amount, &caps.mint_cap);
    ...
    public entry fun register(account: &signer) acquires TokenHolders {
        ...
        coin::register<KaneWilliamson>(account);
    ...
    public entry fun transfer(from: &signer, to: address, amount: u64) acquires TokenHolders {
        coin::transfer<KaneWilliamson>(from, to, amount);
    ...
    public fun balance(addr: address): u64 {
        coin::balance<KaneWilliamson>(addr)
    }
    ...
    public fun get_coin_info(): (string::String, string::String, u8) {
        (
            coin::name<KaneWilliamson>(),
            coin::symbol<KaneWilliamson>(),
            coin::decimals<KaneWilliamson>()
        )
    }
