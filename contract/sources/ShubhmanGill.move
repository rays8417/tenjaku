module higgs::ShubhmanGill {
    ...
    struct CoinCapabilities has key {
        mint_cap: MintCapability<ShubhmanGill>,
        burn_cap: BurnCapability<ShubhmanGill>,
        freeze_cap: FreezeCapability<ShubhmanGill>,
    }
    ...
    fun init_module(admin: &signer) {
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<ShubhmanGill>(
            admin,
            string::utf8(b"ShubhmanGill"),
            string::utf8(b"SGL"),
            8,
            true, // monitor_supply
        );
        ...
        coin::register<ShubhmanGill>(admin);
        ...
        let coins = coin::mint<ShubhmanGill>(initial_supply, &mint_cap);
    ...
    public entry fun mint(admin: &signer, to: address, amount: u64) acquires CoinCapabilities, TokenHolders {
        ...
        let current_supply_option = coin::supply<ShubhmanGill>();
        ...
        let coins = coin::mint<ShubhmanGill>(amount, &caps.mint_cap);
    ...
    public entry fun register(account: &signer) acquires TokenHolders {
        ...
        coin::register<ShubhmanGill>(account);
    ...
    public entry fun transfer(from: &signer, to: address, amount: u64) acquires TokenHolders {
        coin::transfer<ShubhmanGill>(from, to, amount);
    ...
    public fun balance(addr: address): u64 {
        coin::balance<ShubhmanGill>(addr)
    }
    ...
    public fun get_coin_info(): (string::String, string::String, u8) {
        (
            coin::name<ShubhmanGill>(),
            coin::symbol<ShubhmanGill>(),
            coin::decimals<ShubhmanGill>()
        )
    }
