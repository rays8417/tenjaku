...
const PLAYER_MAPPING: Record<string, { name: string; team: string; position: "BAT" | "BWL" | "AR" | "WK"; avatar: string }> = {
  "BenStokes": { name: "Ben Stokes", team: "ENG", position: "AR", avatar: "BS" },
  "TravisHead": { name: "Travis Head", team: "AUS", position: "BAT", avatar: "TH" },
  "ViratKohli": { name: "Virat Kohli", team: "IND", position: "BAT", avatar: "VK" },
  "GlenMaxwell": { name: "Glenn Maxwell", team: "AUS", position: "AR", avatar: "GM" },
  "ShubhamDube": { name: "Shubham Dube", team: "IND", position: "AR", avatar: "SD" },
  "HardikPandya": { name: "Hardik Pandya", team: "IND", position: "AR", avatar: "HP" },
  "ShubhmanGill": { name: "Shubman Gill", team: "IND", position: "BAT", avatar: "SG" },
  "KaneWilliamson": { name: "Kane Williamson", team: "NZ", position: "BAT", avatar: "KW" },
  "AbhishekSharma": { name: "Abhishek Sharma", team: "IND", position: "BAT", avatar: "AS" },
  "JaspreetBumhrah": { name: "Jasprit Bumrah", team: "IND", position: "BWL", avatar: "JB" },
  "SuryakumarYadav": { name: "Suryakumar Yadav", team: "IND", position: "BAT", avatar: "SY" }
};

// Function to create a default player entry for unknown players
function createDefaultPlayer(playerName: string): { name: string; team: string; position: "BAT" | "BWL" | "AR" | "WK"; avatar: string } {
  // Extract initials for avatar
  const words = playerName.match(/[A-Z][a-z]+/g) || [playerName];
  const avatar = words.map(word => word[0]).join('').substring(0, 2);
...
async function fetchTokenPairReserves(): Promise<TokenPairReserve[]> {
  try {
    const response = await fetch('https://fullnode.devnet.aptoslabs.com/v1/accounts/0xaf230e3024e92da6a3a15f5a6a3f201c886891268717bf8a21157bb73a1c027b/resources');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('Total token reserves found:', tokenReserves.length);
        
        const processedHoldings: Holding[] = tokenReserves.map((reserve, index) => {
          console.log(`\n--- Processing reserve ${index + 1} ---`);
          console.log('Full token type:', reserve.type);
          
          const playerName = extractPlayerName(reserve.type);
          console.log('Extracted player name:', playerName);
          
          if (!playerName) {
            console.log('Could not extract player name from:', reserve.type);
            return null;
          }
          ...
          const isBosonFirst = reserve.type.includes('Boson::Boson,') && !reserve.type.includes(', 0xaf230e3024e92da6a3a15f5a6a3f201c886891268717bf8a21157bb73a1c027b::Boson::Boson');
          
          const price = calculateBosonValue(reserve.data.reserve_x, reserve.data.reserve_y, isBosonFirst);
          ...
          const reserveXNum = parseFloat(reserve.data.reserve_x) / Math.pow(10, 8); // Convert from 8 decimals
          const reserveYNum = parseFloat(reserve.data.reserve_y) / Math.pow(10, 8); // Convert from 8 decimals
          ...
          return {
            ...
            change1h: change1h,
            shares: shares,
            holdings: (shares / 100) * totalTokenSupply * price, // Calculate holdings as actual token amount * price
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-8 bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
          ...
          <div className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-gray-200 bg-gray-50">
            ...
            <div className="col-span-2 text-xs uppercase tracking-wider text-gray-500 font-medium text-right">PRICE (BOSON)</div>
            ...
            <div className="col-span-2 text-xs uppercase tracking-wider text-gray-500 font-medium text-right">HOLDINGS (BOSON)</div>
          </div>
                ...
                <div key={holding.id} className="grid grid-cols-12 gap-4 px-6 py-4 hover:bg-gray-50 transition-colors">
                  ...
                  <div className="col-span-2 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <span className="font-medium text-black">{holding.price.toFixed(6)}</span>
                    </div>
                  </div>
                  <div className="col-span-2 text-right">
                    <span className={`text-sm font-medium ${holding.change1h >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                      {holding.change1h >= 0 ? '↑' : '↓'}{Math.abs(holding.change1h).toFixed(2)}%
                    </span>
                  </div>
                  <div className="col-span-2 text-right">
                    <span className="font-medium text-black">{holding.shares.toFixed(2)}%</span>
                  </div>
                  <div className="col-span-2 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <span className="text-yellow-500">🪙</span>
                      <span className="font-medium text-black">{holding.holdings.toFixed(4)}</span>
                    </div>
                  </div>
                </div>
              ))
